...
public class AbarbeitungGUI extends JFrame implements TableModelListener {
    ...
    public AbarbeitungGUI(boolean maximize) {
        ...
        JMenuItem ready = new JMenuItem("markierten Auftrag als \"fertig\" ablegen");
    ...
    protected void unready() {
        if (table.getSelectedRow() != -1 && !String.valueOf(model.getValueAt(table.getSelectedRow(), 4)).equalsIgnoreCase("unfertig")) {
            int really = JOptionPane.showConfirmDialog(AbarbeitungGUI.this, "Wirklich diesen Auftrag als UNERLEDIGT markieren?", "Sicherheitsabfrage", JOptionPane.OK_CANCEL_OPTION);
            if (really == JOptionPane.OK_OPTION) {
                int selrow = getSelectedRow();
